...
beforeEach(function (): void {
    ...
    $this->tool = new DeleteErrorTool($this->mockClient);
});

test('deletes error successfully and returns formatted output', function (): void {
    $this->mockClient->shouldReceive('deleteError')
        ->once()
        ->with('123', 'php')
        ->andReturn([
            'success' => true,
            ...
            'data' => [
                ...
                'activity' => [
                    'id' => 462,
                    'action' => 'archived',
                    'performed_at' => '2026-01-23T12:39:00+00:00',
                ],
            ],
        ]);
    ...
    expect((string) $response->content())
        ->toContain('Error Archived Successfully')
        ->toContain('Error archived successfully')
        ->toContain('archived')
        ->toContain('462');
});
...
test('normalizes error ID by stripping err_ prefix', function (): void {
    $this->mockClient->shouldReceive('deleteError')
        ...
        ->andReturn([
            'success' => true,
            'status' => 200,
            'data' => ['message' => 'Error archived successfully'],
        ]);
...
test('returns generic error for other failures', function (): void {
    ...
    expect((string) $response->content())
        ->toContain('Failed to delete error')
