...
beforeEach(function (): void {
    ...
    $this->tool = new UnignoreErrorTool($this->mockClient);
});

test('unignores error successfully and returns formatted output', function (): void {
    $this->mockClient->shouldReceive('unignoreError')
        ->once()
        ->with('123', 'php')
        ->andReturn([
            'success' => true,
    ...
    expect((string) $response->content())
        ->toContain('Error Unignored Successfully')
        ->toContain('open')
        ->toContain('unignored');
});
...
test('returns error when error_id is missing', function (): void {
    $this->mockClient->shouldNotReceive('unignoreError');
...
test('returns generic error for other failures', function (): void {
    ...
    expect((string) $response->content())
        ->toContain('Failed to unignore error')
...
test('has correct description property', function (): void {
    ...
    expect($description)->toContain('Remove ignore status');
