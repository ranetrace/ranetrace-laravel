...
class UnignoreErrorTool extends Tool
{
     ...
     */
    protected string $description = 'Remove ignore status from an error, returning it to open state. This is an idempotent operation.';
    ...
    public function handle(Request $request): Response
    {
        ...
        $result = $this->client->unignoreError($errorId, $type);

        if (! $result['success']) {
            return $this->handleApiError($result, 'Failed to unignore error');
        }
    ...
    protected function formatResponse(array $data, string $errorId): string
    {
        ...
        $activityAction = $activity['action'] ?? 'unignored';
        $performedAt = $activity['performed_at'] ?? 'N/A';

        return <<<RESPONSE
        # Error Unignored Successfully
