...
class ReopenErrorTool extends Tool
{
    use NormalizesIds;
     ...
     */
    protected string $description = 'Reopen a resolved error, moving it back to open state. This is an idempotent operation.';
    ...
    public function handle(Request $request): Response
    {
        ...
        $result = $this->client->reopenError($errorId, $type);
    ...
    protected function handleErrorResponse(array $result, string $errorId): Response
    {
        ...
        return match ($result['status']) {
            ...
            default => Response::error("Failed to reopen error: {$errorMessage}"),
        };
    }
    ...
    protected function formatResponse(array $data, string $errorId): string
    {
        ...
        $activityAction = $activity['action'] ?? 'reopened';
        $performedAt = $activity['performed_at'] ?? 'N/A';

        return <<<RESPONSE
        # Error Reopened Successfully
