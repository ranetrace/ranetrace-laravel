...
class BulkIgnoreErrorsTool extends Tool
{
     ...
     */
    protected string $description = 'Ignore multiple errors at once. Maximum 50 errors per request. Operations are atomic - all succeed or all fail.';
    ...
    public function handle(Request $request): Response
    {
        ...
        $result = $this->client->bulkIgnoreErrors($normalizedIds, $type);

        if (! $result['success']) {
            return $this->handleApiError($result, 'Failed to bulk ignore errors');
        }
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'error_ids' => $schema->array()
                ->description('Array of error IDs to ignore (max 50). IDs can include err_ prefix.')
                ->required(),
            'type' => $schema->string()
    ...
    {
        $ignoredCount = $data['ignored_count'] ?? 0;
        $errors = $data['errors'] ?? [];
