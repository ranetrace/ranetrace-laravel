...
class BulkRestoreErrorsTool extends Tool
{
     ...
     */
    protected string $description = 'Restore multiple soft-deleted (archived) errors at once. Maximum 50 errors per request. Operations are atomic - all succeed or all fail.';
    ...
    public function handle(Request $request): Response
    {
        ...
        $result = $this->client->bulkRestoreErrors($normalizedIds, $type);
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'error_ids' => $schema->array()
                ->description('Array of error IDs to restore (max 50). IDs can include err_ prefix.')
                ->required(),
            'type' => $schema->string()
    ...
    protected function validateErrorIds($errorIds): ?string
    {
        ...
        if (count($errorIds) > self::MAX_ERRORS) {
            return 'Maximum '.self::MAX_ERRORS.' errors can be restored at once.';
        }
    ...
    protected function handleErrorResponse(array $result): Response
    {
        ...
        return match ($result['status']) {
            ...
            default => Response::error("Failed to bulk restore errors: {$errorMessage}"),
        };
    }
